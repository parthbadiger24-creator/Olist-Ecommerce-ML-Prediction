...
# ── Configuration ────────────────────────────────────────────────────
# Using relative paths dynamically resolved from the script's location.
# This ensures consistent execution in isolated academic environments or cloud syncs.
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# Pointing to the processed datasets created by data_preparation.py
PROCESSED_DIR = os.path.join(SCRIPT_DIR, "data", "processed")
TRAIN_PATH  = os.path.join(PROCESSED_DIR, "train_data.csv")
TEST_PATH   = os.path.join(PROCESSED_DIR, "test_data.csv")
MASTER_PATH = os.path.join(PROCESSED_DIR, "master_olist_data.csv")

# Defining distinct output folders for analytical visualizations and tabular data
FIG_DIR     = os.path.join(SCRIPT_DIR, "results", "figures")
RES_DIR     = os.path.join(SCRIPT_DIR, "results", "tables")
...
def run_corridor_analysis(df):
    ...
    # Construct the Mann–Whitney non-parametric evaluation to test geographic routing friction
...
def run_prediction(train_df, test_df):
    ...
    # Define predictive features optimized logically for delivery modeling
    feature_cols = [
        "distance_km", "actual_delivery_days", "delivery_delay_days",
...
def run_seller_analysis(df):
    ...
    # Segment merchant framework logic drawing from grouped performance parameters
